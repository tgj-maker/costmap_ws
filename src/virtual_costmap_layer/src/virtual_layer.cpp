...
namespace virtual_costmap_layer 
{
    ...
    void VirtualLayer::obstaclesCallback(const virtual_costmap_layer::ObstaclesConstPtr &obstacles_msg)
    {
        ...
        for (int i = 0; i < obstacles_msg->list.size(); ++i) 
        {
            ...
            else if (obstacles_msg->list[i].form.size() == 2) 
            {
                ...
                //获取N的绝对值进行归一化，并将其获取到costmap分辨率的长度
    ...
    void VirtualLayer::updateBounds(double robot_x, double robot_y, double robot_yaw, double *min_x, double *min_y, double *max_x, double *max_y)
    {
        if (!enabled_) {
            return;
        }

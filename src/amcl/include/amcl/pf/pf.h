...
typedef struct _pf_sample_set_t
{
  ...
  // 粒子群
  //cluster_count表示当前粒子群的数量，cluster_max_count表示最大粒子群数量
...
typedef struct _pf_t
{
  ...
  //样本集。我们保留两个集合并使用[ Current _ set ]来标识活动集合。
  // 当前激活态的样本集索引
  int current_set;
  // 样本集合,双缓存
  pf_sample_set_t sets[2];

  //参数alpha_slow和alpha_fast的配置，决定了字段w_slow和w_fast两个平均权重过滤值的衰减速率。
  //这些字段用于触发recover机制，增加随机位姿， 用以削弱粒子滤波器的粒子匮乏问题的影响。
  ...
  // 在pf被认为不收敛的每个轴上的距离阈值,判定粒子集合发散的阈值

...
{
  //atan2(x)函数返回以弧度为单位的角度，范围为[-pi,+pi]，
  //注意是包含-pi和+pi的。函数内部考虑了参数x,y 的符号。所以可以区分坐标点在哪个象限。
  return atan2(sin(z),cos(z));
}

//返回a，b之间的夹角，并且返回值总是落在[-pi,+pi]里。
//总是取从a到b夹角小的那部分，如果从a到b为顺时针旋转返回正值，反之返回负值。
//参考链接：https://blog.csdn.net/shoufei403/article/details/101906644
...
//pf是需要更新的滤波器对象，data则是指示了更新的运动数据，包含pose和delta
  ...
  switch( this->model_type )
  {
      ...
      case ODOM_MODEL_DIFF:
      {
        ...
        for (int i = 0; i < set->sample_count; i++)
        {
          ...
          delta_trans_hat = delta_trans - pf_ran_gaussian(this->alpha3*delta_trans*delta_trans +
      ...
      case ODOM_MODEL_OMNI_CORRECTED:
      {
        ...
        // Precompute a couple of things
        double trans_hat_stddev = sqrt( alpha3 * (delta_trans*delta_trans) + alpha4 * (delta_rot*delta_rot) );
        double rot_hat_stddev = sqrt( alpha1 * (delta_rot*delta_rot) +alpha2 * (delta_trans*delta_trans) );
        double strafe_hat_stddev = sqrt( alpha4 * (delta_rot*delta_rot) +  alpha5 * (delta_trans*delta_trans) );
      ...
      case ODOM_MODEL_DIFF_CORRECTED:
      {
        ...
        for (int i = 0; i < set->sample_count; i++)
        {
          ...
          // Sample pose differences
          delta_rot1_hat = angle_diff(delta_rot1,pf_ran_gaussian(sqrt(this->alpha1*delta_rot1_noise*delta_rot1_noise +
                                                                   this->alpha2*delta_trans*delta_trans)));
          delta_trans_hat = delta_trans -pf_ran_gaussian(sqrt(this->alpha3*delta_trans*delta_trans +
                                                                 this->alpha4*delta_rot1_noise*delta_rot1_noise +
                                                                 this->alpha4*delta_rot2_noise*delta_rot2_noise));
          delta_rot2_hat = angle_diff(delta_rot2, pf_ran_gaussian(sqrt(this->alpha1*delta_rot2_noise*delta_rot2_noise +
                                                                  this->alpha2*delta_trans*delta_trans)));

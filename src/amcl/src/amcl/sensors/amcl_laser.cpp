...
void AMCLLaser::SetModelBeam(double z_hit,double z_short, double z_max,double z_rand,double sigma_hit, double lambda_short, double chi_outlier)
{
...
void AMCLLaser::SetModelLikelihoodFieldProb(double z_hit,double z_rand, double sigma_hit,double max_occ_dist,bool do_beamskip,
...
double AMCLLaser::BeamModel(AMCLLaserData *data, pf_sample_set_t* set)
{
      ...
      {
        sample = set->samples + j;  //sample类似迭代器指针，通过j进行指针位置的移动，指针类型为pf_sample_t
        //这是粒子对应的位姿，是经运动模型更新后先验位姿
        pose = sample->pose;
        ...
        for (i = 0; i < data->range_count; i += step)
        {
          ...
          //pose.v[2]表示机器人在global坐标系下的朝向，obs_bearing表示激光与机器人朝向的夹角
          map_range = map_calc_range(self->map, pose.v[0], pose.v[1],
                                    pose.v[2] + obs_bearing, data->range_max);
          pz = 0.0;

          // Part 1: good, but noisy, hit
          z = obs_range - map_range;    //对应公式z_t_k-z_t_k*
          pz += self->z_hit * exp(-(z * z) / (2 * self->sigma_hit * self->sigma_hit));  //对应公式p_hit

          // Part 2: short reading from unexpected obstacle (e.g., a person)
          if(z < 0)
            pz += self->z_short * self->lambda_short * exp(-self->lambda_short*obs_range);   //对应公式p_short

          // Part 3: Failure to detect obstacle, reported as max-range
          if(obs_range == data->range_max)
            pz += self->z_max * 1.0;      //对应公式p_max

          // Part 4: Random measurements
          if(obs_range < data->range_max)
            pz += self->z_rand * 1.0/data->range_max;    //对应公式p_rand
